...
final class MediaServerTests: XCTestCase {
    ...
    func testAddonMetaEnrichesTheSharedDetailPage() throws {
        var sparse = try JSONDecoder().decode(StremioMeta.self, from: Data(#"""
        {"id":"tt1234567","type":"series","name":"Example","poster":"https://example.test/poster.jpg",
         "videos":[{"id":"tt1234567:1:1","title":"Pilot","season":1,"episode":1}]}
        """#.utf8))
        let rich = try JSONDecoder().decode(StremioMeta.self, from: Data(#"""
        {"id":"tt1234567","type":"series","name":"Example","background":"https://example.test/backdrop.jpg",
         "logo":"https://example.test/logo.png","description":"A series.","releaseInfo":"2012-2017",
         "released":"2012-09-29T00:00:00Z","genres":["Action"],"imdbRating":"7.6",
         "country":"United States","cast":["First Actor","Second Actor"],"director":["Director"],
         "trailers":[{"source":"dQw4w9WgXcQ","type":"Trailer"}]}
        """#.utf8))

        sparse.fillMissing(from: rich)
        let item = MediaItem(meta: sparse, addonID: "catalog-addon")

        XCTAssertEqual(item.childCount, 1)
        XCTAssertEqual(item.communityRating, 7.6)
        XCTAssertEqual(item.people?.map(\.name), ["First Actor", "Second Actor", "Director"])
        XCTAssertEqual(item.remoteTrailers?.first?.thumbnailURL?.host, "img.youtube.com")
        XCTAssertEqual(item.productionLocations, ["United States"])
        XCTAssertNotNil(item.formattedAirDate)
        XCTAssertEqual(item.logoArtworkURL, "https://example.test/logo.png")
    }

    func testAddonMetaLinksCanSupplyCastWhenLegacyCastIsAbsent() throws {
        let meta = try JSONDecoder().decode(StremioMeta.self, from: Data(#"""
        {"id":"movie","type":"movie","name":"Example",
         "links":[{"name":"Actor","category":"actor","url":"https://example.test/actor"},
                  {"name":"Director","category":"director","url":"https://example.test/director"}],
         "trailers":[{"ytId":"dQw4w9WgXcQ"}]}
        """#.utf8))
        let item = MediaItem(meta: meta, addonID: "addon")
        XCTAssertEqual(item.people?.map(\.name), ["Actor", "Director"])
        XCTAssertEqual(item.remoteTrailers?.first?.name, "Trailer")
    }

    func testAddonRuntimeReadsHoursAndMinutes() {
        XCTAssertEqual(MediaItem.ticks(fromRuntime: "1h 38m"), 98 * 600_000_000)
        XCTAssertEqual(MediaItem.ticks(fromRuntime: "1 hour 38 minutes"), 98 * 600_000_000)
        XCTAssertEqual(MediaItem.ticks(fromRuntime: "142 min"), 142 * 600_000_000)
        XCTAssertEqual(MediaItem.ticks(fromRuntime: "98"), 98 * 600_000_000)
    }

    func testSearchOnlyAddonCatalogIsDiscoverableButNotAShelf() throws {
        let manifest = try JSONDecoder().decode(StremioManifest.self, from: Data(#"""
        {"id":"search-addon","name":"Search","resources":["catalog","meta"],"types":["movie"],
         "catalogs":[{"type":"movie","id":"search","name":"Search Movies",
                      "extra":[{"name":"search","isRequired":true}]}]}
        """#.utf8))
        let addon = StremioAddon(manifest: manifest, address: "https://example.test")

        XCTAssertTrue(addon.catalogs.isEmpty)
        XCTAssertEqual(manifest.catalogs.count, 1)
        XCTAssertTrue(manifest.catalogs[0].supportsSearch)
        XCTAssertFalse(manifest.catalogs[0].isBrowsable)
    }

    @MainActor
    func testSearchFindsLoadedAddonTitlesWithoutASearchEndpoint() async throws {
        let defaults = try XCTUnwrap(UserDefaults(suiteName: "lineup-search-\(UUID().uuidString)"))
        let library = MediaLibrary(defaults: defaults)
        let root = MediaItem(id: "catalog", name: "Trakt List", type: "CollectionFolder",
            overview: nil, productionYear: nil, primaryImageAspectRatio: nil, childCount: nil)
        let title = MediaItem(id: "addon:film", name: "The Whisper Man", type: "Movie",
            overview: nil, productionYear: 2026, primaryImageAspectRatio: nil, childCount: nil)
        library.addonShelves = [MediaCatalog(root: root, items: [title])]

        let results = try await library.search("whisper")
        XCTAssertEqual(results.map(\.name), ["The Whisper Man"])
    }
